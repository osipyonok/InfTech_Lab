...
QByteArray DataBaseConnection::getPossibleIntersectionTables(QString tbl){
    QJsonObject tot , obj;
    DataTable cur_table = this->db->getDataTable(tbl);
    QJsonArray arr;
    for(QString tn : this->db->getTableNamesSet()){
        DataTable cur = this->db->getDataTable(tn);
        if(cur_table == cur && tbl != cur.getName()){
            arr.append(QJsonValue(cur.getName()));
        }
    }
    obj.insert("Result" , QJsonValue(true));
    obj.insert("Tables" , QJsonValue(arr));
    tot.insert("getPossibleIntegsection" , QJsonValue(obj));
    return QJsonDocument(tot).toJson();
}

QByteArray DataBaseConnection::makeIntersection(QJsonArray arr){
    QJsonObject tot , obj;
    vector<QString> v;
    for(QJsonValue val : arr)v.push_back(val.toString());
    DataTable t = this->db->Intersect(v);
    obj.insert("Result" , QJsonValue(true));
    obj.insert("Table" , QJsonValue(t.toJson()));
    tot.insert("makeIntersection" , QJsonValue(obj));
    return QJsonDocument(tot).toJson();
}
