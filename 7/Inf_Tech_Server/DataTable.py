...
class DataTable:
    ...
    def __eq__(self, other):
        ...
        for i in range(0, len(self.descriptions)):
            res = res and (self.descriptions[i][0] == other.descriptions[i][0])
            print(self.descriptions[i], other.descriptions[i])
            print()
            res = res and (self.descriptions[i][1] == other.descriptions[i][1])
        return res
    ...
    @staticmethod
    def Intersect(tables):
        res = DataTable()
        res.name = "Intersection"
        if len(tables) == 0:
            return res
        res.descriptions = tables[0].descriptions
        res.field_names = tables[0].field_names
        rws = tables[0].getRows()
        was = [True] * len(rws)
        for i in range(0, len(rws)):
            for j in range(1, len(tables)):
                tmp = tables[j].getRows()
                ck = False
                for o in tmp:
                    if o.__eq__(rws[i]):
                        ck = True
